...
void Server::getPoolStatus(const HttpRequestPtr&, std::function<void(const HttpResponsePtr&)>&& callback) {
    try {
        ...
        if (!entries.has_value())
            throw std::runtime_error(std::format("message: {},code:{}",
                entries.error().message, entries.error().code));
        ...
        if (!sessionEntries.has_value())
            throw std::runtime_error(std::format("message: {},code:{}",
                sessionEntries.error().message, sessionEntries.error().code));

        // Build JSON using utility
        Json::Value json = Raptor::Core::Api::Utils::ToPoolStatusJson(
            pool,
            entries.value(),
...
void Server::createServer(const HttpRequestPtr& req, std::function<void(const HttpResponsePtr&)>&& callback) {
    try {
        ...
        auto ok = servers.createServer(serverType.value(), config);
