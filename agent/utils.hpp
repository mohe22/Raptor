...
inline void scopy(char* dst, size_t dsz, const char* src) noexcept {
    if (!src) return;
    std::snprintf(dst, dsz, "%s", src);
}

inline void scopy(char* dst, size_t dsz, std::string_view src) noexcept {
    std::snprintf(dst, dsz, "%.*s", static_cast<int>(src.size()), src.data());
}

inline const char* env(std::initializer_list<const char*> keys) noexcept {
...
[[nodiscard]] inline Raptor::Common::Register collect() noexcept {
    ...
    // process path + name
    {
        ...
        if (len > 0) {
            ...
            scopy(reg.processName, sizeof(reg.processName), slash ? slash + 1 : reg.processPath);
        }
    }
    ...
    // dns
    {
        ...
        for (std::string line; std::getline(f, line);) {
            if (line.starts_with("nameserver ")) {
                scopy(reg.dnsServer, sizeof(reg.dnsServer), std::string_view(line).substr(11));
                break;
            }
        }
    }
